...
caminho = sorted(Path(diario_diretorio).glob('**/*'))
###print(str(caminho))
for arquivo in caminho:
    if Path.is_file(arquivo):
        ####print(str(arquivo) + ': file')

        # Arquivo temporario para gravação da primeira linha
        arquivo_tmp = str(arquivo) + '.tmp'

        # Analisa a primeira linha
        arquivo_open = open(arquivo, 'r+')
        linha = arquivo_open.readline()

        # A primeira linha é igual ao caminho completo do arquivo?
        if str(linha) == str(arquivo):
            print('linhas iguais:' + str(linha) )
        else:
            print('linhas iguais:' + str(linha) )

        ###arquivo_open.write(str(arquivo) + '\n')
        ###arquivo_open.close()
 
        exit(0)

    ##if Path.is_dir(arquivo):
    ##    print(str(arquivo) + ': dir')
